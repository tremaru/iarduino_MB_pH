#include "iarduino_MB_pH.h"			//	╔═══════════════════════╗
									...
									//	║ ERR_pH    │ --5-----  ║ Флаг указывает на то, что показания pH "скачут".
									//	║ ERR_Vout  │ ---4----  ║ Флаг первышения напряжения на выходе ОУ (pH < 0 ).
									//	║ ERR_Vout  │ ----3---  ║ Флаг занижения  напряжения на выходе ОУ (pH > 14).
									//	║ ERR_Vo    │ -----2--  ║ Флаг первышения напряжения подаваемого на датчик (Vo > 0.5Vcc/Ky + 0.1).
									//	║ ERR_Vo    │ ------1-  ║ Флаг занижения  напряжения подаваемого на датчик (Vo < 0.5Vcc/Ky - 0.1).
...
#define AO_KNOWN_PH_2	1			//	║ AO_KNOWN_PH_2 ║ 09.180║ Известное pH, для 2 стадии калибровки.
									//	╟───────────────╫───────╢
#define AO_KY			2			//	║ AO_KY         ║ 03.000║ Коэффициент усиления усилителя в тысячных долях. Ky = 1 + R4/R5 = 1 + 2к/1к = 3.000
#define AO_VSTP			3			//	║ AO_VSTP       ║ 059.16║ Шаг смещения U датчика на 1pH  в сотых долях мВ. Смещение напряжения датчиком на 1 Vstp соответствует изменению кислотности на 1 pH.
#define AO_PHN			4			//	║ AO_PHN        ║ 07.000║ Нейтральная кислотность датчика в тысячных долях. Кислотность жидкости при которой отсутствует смещение напряжения датчиком.
									//	╟───────────────╫───────╢
#define AO_CALC_SAVE	5			//	║ AO_CALC_SAVE  ║   0   ║ Код разрешения записи калибровочных значений в Flash память. Сбрасывается автоматически. Должен быть установлен в значение (0x2709) при запуске калибровки и до записи калибровочных значений напрямую через регистры (без калибровки).
									//	╟───────────────╫───────╢
...
#define AI_VN			3			//	║ AI_VN         ║   0   ║ Нейтральное напряжение на выходе усилителя в десятитысячных долях В. ( 0...6,5535В ). Показывает каким должно быть Vout при отсутствии смещения напряжения датчиком.
									//	╟───────────────╫───────╢
#define AI_PH			4			//	║ AI_PH         ║   0   ║ Кислотность жидкости в тысячных долях pH ( 0...65,535 pH ). pH = (Vn-Vout)/(Vstp*Ky) + pHn
									//	╚═══════════════╩═══════╝
																															//
//		КОНСТРУКТОР КЛАССА:																									//
...
float	iarduino_MB_pH::getKnownPH(uint8_t stage){																			//	stage - стадия 1 или 2.
			objModbus->codeError=ERROR_SYNTAX; if( !valID ){ return -1.0f; }												//	Ошибка синтаксиса, модуль не инициализирован.
			float i= -1.0f;																									//
			if( stage==1 ){ i=(float)objModbus->holdingRegisterRead(valID, AO_KNOWN_PH_1); }								//	Читаем регистр "Holding Register" AO[AO_KNOWN_PH_1]. Функция возвращает прочитанное значение (0...65535), или -1 при неудаче.
			if( stage==2 ){ i=(float)objModbus->holdingRegisterRead(valID, AO_KNOWN_PH_2); }								//	Читаем регистр "Holding Register" AO[AO_KNOWN_PH_2]. Функция возвращает прочитанное значение (0...65535), или -1 при неудаче.
...
bool	iarduino_MB_pH::setKnownPH(uint8_t stage, float pH){																//	stage - стадия 1 или 2, pH - кислотность от 0.000 до 14.000.
			...
			if( !objModbus->holdingRegisterWrite(valID, AO_CALC_SAVE, 0x2709) ){ return false; }							//	Записываем значение 0x2709 в регистр "Holding Register" AO[AO_CALC_SAVE]. Функция возвращает 1 при успехе, 0 при неудаче.
			if( stage==1 ){ i = objModbus->holdingRegisterWrite(valID, AO_KNOWN_PH_1, (uint16_t)(pH*1000.0) ); }			//	Записываем в регистр "Holding Register" AO[AO_KNOWN_PH_1] кислотность в тысячных долях pH. Функция возвращает 1 при успехе, 0 при неудаче.
			if( stage==2 ){ i = objModbus->holdingRegisterWrite(valID, AO_KNOWN_PH_2, (uint16_t)(pH*1000.0) ); }			//	Записываем в регистр "Holding Register" AO[AO_KNOWN_PH_2] кислотность в тысячных долях pH. Функция возвращает 1 при успехе, 0 при неудаче.
			delay(50);																										//	Ждём 50 мс, пока модуль не сохранит новые данные в flash память.
			return i;																										//
}																															//
																															//
//		ФУНКЦИЯ ПОЛУЧЕНИЯ КОЭФФИЦИЕНТА УСИЛЕНИЯ УСИЛИТЕЛЯ:																	//	Возвращает коэффициент усиления ОУ, или -1 при провале чтения.
float	iarduino_MB_pH::getKy(void){																						//
			objModbus->codeError=ERROR_SYNTAX; if( !valID ){ return -1.0f; }												//	Ошибка синтаксиса, модуль не инициализирован.
			float i=(float)objModbus->holdingRegisterRead(valID, AO_KY);													//	Читаем коэффициент усиления из регистра "Holding Register" AO[AO_KY]. Функция возвращает прочитанное значение (0...65535), или -1 при неудаче.
			if( i>=0 ){ i/=1000.0f; } return i;																				//	Возврашаем коэффициент усиления полученный в тысячных долях.
}																															//
																															//
//		ФУНКЦИЯ УСТАНОВКИ КОЭФФИЦИЕНТА УСИЛЕНИЯ УСИЛИТЕЛЯ:																	//	Возвращает результат записи коэффициента усиления: true-успех / false-провал.
bool	iarduino_MB_pH::setKy(float gain){																					//	gain - коэффициент усиления от 1.000 до 65.535.
			objModbus->codeError=ERROR_SYNTAX; if( !valID ){ return false; }												//	Ошибка синтаксиса, модуль не инициализирован.
			if( gain<1.0f || gain>65.535f ){ return false; }																//	Возвращаем флаг ошибки синтаксиса.
			if( !objModbus->holdingRegisterWrite(valID, AO_CALC_SAVE, 0x2709) ){ return false; }							//	Записываем значение 0x2709 в регистр "Holding Register" AO[AO_CALC_SAVE]. Функция возвращает 1 при успехе, 0 при неудаче.
			bool i = objModbus->holdingRegisterWrite(valID, AO_KY, (uint16_t)(gain*1000.0) );								//	Записываем в регистр "Holding Register" AO[AO_KY] коэффициент усиления в тысячных долях. Функция возвращает 1 при успехе, 0 при неудаче.
			delay(50); 																										//	Ждём 50 мс, пока модуль не сохранит новые данные в flash память.
			return i;																										//
}																															//
																															//
...
float	iarduino_MB_pH::getVstp(void){																						//
			objModbus->codeError=ERROR_SYNTAX; if( !valID ){ return -1.0f; }												//	Ошибка синтаксиса, модуль не инициализирован.
			float i=(float)objModbus->holdingRegisterRead(valID, AO_VSTP);													//	Читаем напряжение из регистра "Holding Register" AO[AO_VSTP]. Функция возвращает прочитанное значение (0...65535), или -1 при неудаче.
...
bool	iarduino_MB_pH::setVstp(float v){																					//	v - напряжение от 0.01 мВ до 655.35 мВ.
			...
			if( !objModbus->holdingRegisterWrite(valID, AO_CALC_SAVE, 0x2709) ){ return false; }							//	Записываем значение 0x2709 в регистр "Holding Register" AO[AO_CALC_SAVE]. Функция возвращает 1 при успехе, 0 при неудаче.
			bool i = objModbus->holdingRegisterWrite(valID, AO_VSTP, (uint16_t)(v*100.0) );									//	Записываем в регистр "Holding Register" AO[AO_VSTP] напряжение в сотых долях В. Функция возвращает 1 при успехе, 0 при неудаче.
			delay(50); 																										//	Ждём 50 мс, пока модуль не сохранит новые данные в flash память.
			return i;																										//
}																															//
																															//
...
float	iarduino_MB_pH::getVin(void){																						//
			objModbus->codeError=ERROR_SYNTAX; if( !valID ){ return -1.0f; }												//	Ошибка синтаксиса, модуль не инициализирован.
			float i=(float)objModbus->inputRegisterRead(valID, AI_V0);														//	Читаем напряжение из регистра "Input Register" AI[AI_V0]. Функция возвращает прочитанное значение (0...65535), или -1 при неудаче.
			if( i>=0 ){ i/=10000.0f; } return i;																			//	Возврашаем напряжение полученное в десятитысячных долях В.
}																															//
																															//
//		ФУНКЦИЯ ПОЛУЧЕНИЯ НАПРЯЖЕНИЯ НА ВЫХОДЕ УСИЛИТЕЛЯ:																	//	Возвращает напряжение на выходе ОУ, или -1 при провале чтения.
float	iarduino_MB_pH::getVout(void){																						//
			objModbus->codeError=ERROR_SYNTAX; if( !valID ){ return -1.0f; }												//	Ошибка синтаксиса, модуль не инициализирован.
			float i=(float)objModbus->inputRegisterRead(valID, AI_VOUT);													//	Читаем напряжение из регистра "Input Register" AI[AI_VOUT]. Функция возвращает прочитанное значение (0...65535), или -1 при неудаче.
			if( i>=0 ){ i/=10000.0f; } return i;																			//	Возврашаем напряжение полученное в десятитысячных долях В.
}																															//
																															//
//		ФУНКЦИЯ ПОЛУЧЕНИЯ НЕЙТРАЛЬНОГО НАПРЯЖЕНИЯ НА ВЫХОДЕ УСИЛИТЕЛЯ:														//	Возвращает нейтральное напряжение на выходе ОУ, или -1 при провале чтения.
float	iarduino_MB_pH::getVn(void){																						//
			objModbus->codeError=ERROR_SYNTAX; if( !valID ){ return -1.0f; }												//	Ошибка синтаксиса, модуль не инициализирован.
			float i=(float)objModbus->inputRegisterRead(valID, AI_VN);														//	Читаем напряжение из регистра "Input Register" AI[AI_VN]. Функция возвращает прочитанное значение (0...65535), или -1 при неудаче.
			if( i>=0 ){ i/=10000.0f; } return i;																			//	Возврашаем напряжение полученное в десятитысячных долях В.
}																															//
																															//
...
float	iarduino_MB_pH::getPHn(void){																						//
			objModbus->codeError=ERROR_SYNTAX; if( !valID ){ return -1.0f; }												//	Ошибка синтаксиса, модуль не инициализирован.
			float i=(float)objModbus->holdingRegisterRead(valID, AO_PHN);													//	Читаем кислотность из регистра "Holding Register" AO[AO_PHN]. Функция возвращает прочитанное значение (0...65535), или -1 при неудаче.
			if( i>=0 ){ i/=1000.0f; } return i;																				//	Возврашаем кислотность полученную в тысячных долях pH.
}																															//
																															//
...
bool	iarduino_MB_pH::setPHn(float pH){																					//	pH - кислотность от 0.000 до 14.000.
			...
			if( pH<0.0f || pH>14.0f ){ return false; }																		//	Возвращаем флаг ошибки синтаксиса.
			if( !objModbus->holdingRegisterWrite(valID, AO_CALC_SAVE, 0x2709) ){ return false; }							//	Записываем значение 0x2709 в регистр "Holding Register" AO[AO_CALC_SAVE]. Функция возвращает 1 при успехе, 0 при неудаче.
			bool i = objModbus->holdingRegisterWrite(valID, AO_PHN, (uint16_t)(pH*1000.0) );								//	Записываем в регистр "Holding Register" AO[AO_PHN] кислотность в тысячных долях pH. Функция возвращает 1 при успехе, 0 при неудаче.
			delay(50); 																										//	Ждём 50 мс, пока модуль не сохранит новые данные в flash память.
			return i;																										//
}																															//
																															//
...
float	iarduino_MB_pH::getPH(void){																						//
			objModbus->codeError=ERROR_SYNTAX; if( !valID ){ return -1.0f; }												//	Ошибка синтаксиса, модуль не инициализирован.
			float i=(float)objModbus->inputRegisterRead(valID, AI_PH);														//	Читаем кислотность из регистра "Input Register" AI[AI_PH]. Функция возвращает прочитанное значение (0...65535), или -1 при неудаче.
			if( i>=0 ){ i/=1000.0f; } return i;																				//	Возврашаем кислотность полученную в тысячных долях pH.
}																															//
